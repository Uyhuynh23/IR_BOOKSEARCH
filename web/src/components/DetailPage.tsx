...
export default function DetailPage({ book, relatedBooks = [], onBack, onSelectBook }: DetailPageProps) {
  const [activeTab, setActiveTab] = useState<'description' | 'metadata' | 'reviews'>('description');
  const [isSaved, setIsSaved] = useState(false);

  const genres = book.google_category 
    ? book.google_category.split(/[,;]/).map(g => g.trim()).filter(Boolean)
    : [];
          ...
          <div>
            ...
            <div style={{
              ...
              {[...Array(5)].map((_, i) => (
                <span key={i} style={{
                  fontSize: '1.5rem',
                  filter: i < Math.floor(book.average_rating) ? 'none' : 'grayscale(100%)',
        ...
        <div style={{ marginBottom: '3rem' }}>
          ...
          {activeTab === 'description' && (
            <div style={{
              ...
              <p style={{ marginBottom: '1rem' }}>
                {book.description || `The Tale of Kieu is an epic poem in Vietnamese written by Nguyen Du (1766-1820), and is widely regarded as the most significant work of Vietnamese literature. It tells of the life, trials and tribulations of Thuy Kieu, a beautiful and talented young woman, who had to sacrifice herself to save her family.`}
              </p>
              <p style={{ marginBottom: '1rem' }}>
              ...
              <p>
                This edition features a new translation that captures the lyrical beauty of the original verses while making the story accessible to modern readers. It includes detailed footnotes explaining cultural references and historical context of the Le Dynasty.
              </p>
                ...
                <div style={{
                  ...
                  {book.extracted_characters && (
                    <div style={{ marginBottom: '0.75rem' }}>
                      ...
                      <span style={{ color: '#6B7280' }}>{book.extracted_characters}</span>
                    </div>
                  )}
                  {book.extracted_settings && (
                    <div>
                      <strong style={{ color: '#2B2B2B' }}>Settings:</strong>{' '}
                      <span style={{ color: '#6B7280' }}>{book.extracted_settings}</span>
                    </div>
                  )}
                </div>
              )}
          ...
          {activeTab === 'metadata' && (
            <div style={{
              ...
              <MetadataItem label="Published Year" value={book.published_year || 'Unknown'} />
              <MetadataItem label="ISBN" value={book.clean_isbn || 'Not available'} />
              <MetadataItem label="Category" value={book.google_category || 'General'} />
              <MetadataItem label="Average Rating" value={book.average_rating.toString()} />
              <MetadataItem label="Preview Link" value={book.preview_link ? 'Available' : 'Not available'} />
            </div>
          )}
          ...
          {activeTab === 'reviews' && (
            <div style={{ textAlign: 'center', padding: '3rem', color: '#6B7280' }}>
              ...
              <p>Check back later to see what readers are saying about this book.</p>
            </div>
          )}
        </div>
              ...
              {relatedBooks.slice(0, 5).map((relatedBook) => (
                ...
                >
                  <div style={{
                    ...
                    <img
                      src={relatedBook.thumbnail || 'https://via.placeholder.com/200x300?text=No+Cover'}
                  ...
                  <div style={{ padding: '1rem' }}>
                    ...
                    <div style={{
                      ...
                      <span style={{ fontSize: '0.85rem', color: '#2B2B2B', fontWeight: 600 }}>
